    ...
    <div class="comment">
      ...
      <div class="comment_item">
        ...
        <div class="comment_item_text">
          ...
          <p style="font-size: 24px; font-weight: bold">haoyunlai</p>
        </div>
      ...
      <hr style="height: 0.1px; background-color: rgb(214, 211, 211)" />
...
.sing {
  ...
  align-items: center;
  height: 100%;
  background-color: rgba(255, 255, 255, 0.1); /* 更强的白色背景 */
  z-index: 1; /* 确保在中间层 */
position: relative; /* 确保内容处于最上层 */
...
.blurred-image {
  ...
  top: 20px; /* 距离上边20px */
  right: 20px; /* 距离右边20px */
  width: calc(100% - 30px); /* 宽度为100%减去左右边距 */
  height: calc(100% - 30px); /* 高度为100%减去上下边距 */
  object-fit: cover; /* 保持图像比例并填充 */
  filter: blur(20px); /* 只在图片上应用模糊效果 */
  opacity: 0.2;
  z-index: 0; /* 确保图片在最底层 */
}
...
.scroll-container {
  ...
  height: 696px; /* 确保容器有固定高度 */
  max-width: 100%; /* 确保宽度不会超出 */
  position: relative; /* 确保文字内容不会被遮挡 */
}
