    ...
    <li v-for="(item, index) in commentList" :key="index">
      ...
      <div ref="up" class="comment-ctr" @click="setSupport(item.id, item.up, userId)">
        <div style="display: flex; align-items: center;">
          <img width="20" src="/src/assets/images/点赞.png" alt="Sidebar" class="icon" />
          {{ item.up }}
          <!-- <yin-icon :icon="iconList.Support"></yin-icon> {{ item.up }}</div> -->
        </div>
...
async function getComment(id) {
  try {
    const result = (await HttpManager.getAllComment(type.value, id));
...
async function submitComment() {
  ...
  const result = (await HttpManager.setComment({ userId: userId.value, content, songId, songListId, nowType }));
  ElMessage({
...
async function setSupport(id, up, userId) {
  ...
  const r = (await HttpManager.testAlreadySupport({ commentId, userId }));
  ElMessage({
    message: r.message,
    type: r.type,
    date: r.data,
